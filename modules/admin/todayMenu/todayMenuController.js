...
const deleteTodayMenu = async (req, res, next) => {
  ...
  try {
    let isDailyMenuExists = await dailyMenuModel.findOne({ _id: todayMenuId });
    if (isDailyMenuExists) {
      let menuDetails = await menuModel.findOne({ _id: isDailyMenuExists.type });
      await dailyMenuModel.findByIdAndRemove(todayMenuId);
      return res.status(200)
          .json({
            statusCode: 200, message: `Today ${menuDetails.title} menu deleted successfully` });
    } else {
      return res.status(500)
          .json({
            statusCode: 500, message: "Invalid today menu id" });
    }
  } catch (err) {
    res.status(500)
          .json({
...
if (error.length != 0) {
  ...
  sub_menu_items = sub_menu_items.filter( function( item, index, inputArray ) {
    ...
    if (dailyMenu) {
      return res.status(400)
          .json({
            statusCode: 400, message: `Menu for ${typeExists.title} is already created for today.` });
    }
    // Check if menu_items are valid
    for (let i = 0; i < sub_menu_items.length; i++) {
      ...
      if (!isSubMenuIdValid) {
        return res.status(400)
          .json({
            statusCode: 400, error: `Item you have added in today's menu is invalid.` });
      }
    }
    const data = {
      sub_menu_items: sub_menu_items,
...
const listTodayMenu = async (req, res, next) => {
  try {
    ...
    const todayDate = date || new Date().toLocaleDateString().replace(/\//g, '-');
    const message = date ? `Menu items for the date: ${date} fetched successfully.` : "Menu items for today are fetched successfully";
    const menus = await menuModel.find({}, '_id time title createdAt updatedAt');
    const result = [];
    for (const menu of menus) {
      const items = await dailyMenuModel.findOne({ type: menu._id, date: todayDate }, 'sub_menu_items type date time createdAt updatedAt');
    
      const subItems = items ? await subMenuModel.find({ _id: { $in: items.sub_menu_items } }, 'item_name price quantity createdAt updatedAt') : [];
...
module.exports = { addTodayMenu, listTodayMenu ,updateTodayMenu, deleteTodayMenu};
